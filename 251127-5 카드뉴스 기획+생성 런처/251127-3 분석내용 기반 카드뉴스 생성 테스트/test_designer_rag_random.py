...
def find_best_reference_images(index, query_text, page_type, final_k=2):
    ...
    try:
        ...
        # 3. 후보군(Pool) 확보: 상위 100개 (다양성 극대화)
        # 100개면 충분히 넓어서, 베이지색이 20개 있어도 다른 게 80개나 됨.
        pool_size = min(len(valid_indices), 100)
        top_indices_pool = np.argsort(scores)[-pool_size:][::-1]

        # 4. 그냥 랜덤 선택 (Simple is Best)
        # 상위 100등 안에 든 녀석들이라면 퀄리티는 보장됨. 그 중에서 운 좋은 2놈 선발.
        ...
        for i in selected_indices:
            ...
            if img_path.exists():
                # 점수는 굳이 출력 안 해도 됨 (어차피 랜덤이니까)
                print(f"- [선택됨] {img_path.name} (톤: {data.get('tone_and_manner')})")
                try:
...
def generate_image_with_retry(prompt_parts, output_path, aspect_ratio):
    """이미지를 생성하며, 500 에러 시 재시도 옵션을 제공합니다."""
...
def run_rag_designer():
    print("🚀 [RAG 기반] 스마트 AI 디자이너를 실행합니다... (다양성 강화 버전)\n")
    ...
    # ⭐⭐⭐ [변경] 레퍼런스 캐싱 제거 - 페이지마다 새로운 선택 ⭐⭐⭐
    # (다양성 극대화: 같은 타입이어도 매번 다른 레퍼런스를 선택)
    ...
    for i, page in enumerate(pages_to_generate):
        ...
        # ⭐ 페이지마다 새로 검색 (이전의 캐싱 제거)
        query_text = f"{tone} 느낌의 {page['type']} 디자인 스타일"
        ref_images = find_best_reference_images(index, query_text, page['type'], final_k=2)
        ...
        if character_image:
            ...
            prompt_parts.append("✅ 규칙 2: 참조 이미지에 캐릭터가 있으면 → 제공된 매돌이 이미지를 그대로 사용하세요.")
            prompt_parts.append("✅ 규칙 3: 참조 이미지에 캐릭터가 없으면 → 어떤 캐릭터도 추가하지 마세요.")
            prompt_parts.append("❌ 금지 사항: 절대로 새로운 캐릭터를 그리지 마세요. 절대로 제공된 매돌이를 변형하지 마세요.")
            prompt_parts.append("❌ 금지 사항: 색상 변경, 모양 변경, 표정 변경, 옷 변경 등 어떤 형태의 변형도 금지됩니다.")
            prompt_parts.append("❌ 금지 사항: 비슷한 다른 캐릭터를 대신 그리는 것도 금지됩니다.")
            prompt_parts.append("⚠️ 경고: 위 규칙을 어기면 설계 의도에 심각한 오류가 발생합니다. 반드시 규칙을 따르세요.")
            prompt_parts.append(character_image) # 이미지는 정확한 참조를 위해 제공
        else:
             prompt_parts.append("\n**[캐릭터 금지 지시사항]**")
