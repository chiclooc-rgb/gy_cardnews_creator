...
def search_rag_references(index, query_text, page_type=None, top_k=3, color_filter=None):
    """RAG 검색 (대소문자 무시 & 색상 필터링 지원)

    Args:
        index: 검색 인덱스
        query_text: 검색 쿼리
        page_type: 페이지 타입 필터 (COVER, BODY, OUTRO)
        top_k: 반환할 상위 결과 개수
        color_filter: 색상 팔레트 필터 (cover_color_tone)
    """
    try:
        # 1. 쿼리 임베딩
        ...
        for i, entry in enumerate(index):
            ...
            # 색상 팔레트 필터링 (필수)
            if color_filter:
                entry_color = str(entry['data'].get('color_palette_feel', '')).strip().lower()
                filter_color = str(color_filter).strip().lower()
                # 색상 팔레트가 유사한지 확인 (처음 10글자 비교하거나 핵심 색상명 포함 여부)
                if not (entry_color == filter_color or
                        any(color_word in entry_color for color_word in filter_color.split(',')[:2])):
                    scores.append(-1)
                    continue
...
class CardNewsLauncher:
    def __init__(self, root):
        ...
        self.detail_level = tk.StringVar(value="detailed")  # 'simple' or 'detailed'

        # 디자인 프로세스 제어용 변수
        self.stop_design_flag = False
        self.design_thread = None
        self.ask_per_page = False
        self.generated_images = []  # 생성된 이미지 경로 저장
    ...
    def create_widgets(self):
        ...
        # 상세도 선택 옵션
        frame_detail = ttk.Frame(step1_frame)
        frame_detail.pack(fill="x", padx=5, pady=10)

        ttk.Label(frame_detail, text="📋 기획 상세도: ").pack(side="left", padx=5)
        ttk.Radiobutton(frame_detail, text="간단하게 (1~2장)", variable=self.detail_level, value="simple").pack(
            side="left", padx=10
        )
        ttk.Radiobutton(frame_detail, text="자세하게 (여러 장)", variable=self.detail_level, value="detailed").pack(
            side="left", padx=10
        )
    ...
    def process_planning(self):
        ...
        try:
            ...
            if detail_level == "simple":
                structure_instruction = "1. **구조 판단:** 내용을 최대한 압축하여 SINGLE(1장) 또는 간단한 MULTI(표지+1~2개 본문)로 제한하세요. 핵심만 담고 부가정보는 제외합니다."
                content_instruction = "2. **내용 요약:** 매우 간단하고 임팩트 있게 요약하세요. 불필요한 세부사항은 제외하고, 가장 중요한 정보 1~2개만 강조하세요."
                log_msg = "📋 [간단 모드] 1~2장으로 압축된 기획안을 작성합니다..."
            else:
                structure_instruction = "1. **구조 판단:** 내용이 단순하면 SINGLE(1장), 복잡하면 MULTI(표지-본문-마무리) 구조로 판단하세요. 필요한 만큼 상세하게 설명합니다."
                content_instruction = "2. **내용 요약:** 핵심 정보를 누락 없이, 카드뉴스에 적합한 짧고 명확한 문장으로 요약하세요."
                log_msg = "📋 [상세 모드] 전체 내용을 담은 기획안을 작성합니다..."
  ...
  "estimated_tone": "(예: 활기찬, 정보성)"
            ...
            # UI 업데이트
            self.root.after(0, lambda: self.update_ui_after_plan(Path(save_path).name))
    ...
    def json_to_readable_text(self, data):
        """JSON을 읽기 좋은 텍스트 형식으로 변환"""
        lines = []

        def add_section(title, value, indent=0):
            prefix = "  " * indent
        ...
        if isinstance(data, dict):
            if 'structure_type' in data:
                lines.append("=" * 60)
                lines.append("📰 카드뉴스 기획안")
                lines.append("=" * 60)
                lines.append(f"구조: {data.get('structure_type', 'N/A')}")
                lines.append(f"톤: {data.get('estimated_tone', 'N/A')}")
                lines.append("")
                ...
                # 표지
                if 'cover' in plan:
                    lines.append("🎯 [표지 (COVER)]")
                    cover = plan['cover']
                    lines.append(f"  메인 제목: {cover.get('main_title', 'N/A')}")
                    if cover.get('sub_title'):
                        lines.append(f"  부제목: {cover.get('sub_title', '')}")
                    lines.append("")

                # 본문
                if 'body' in plan:
                    lines.append("📝 [본문 (BODY)]")
    ...
    def open_json_editor(self):
        ...
        # 1. 보기 탭 (읽기 쉬운 텍스트 형식)
        frame_readable = ttk.Frame(notebook)
        notebook.add(frame_readable, text="📖 보기")

        text_readable = scrolledtext.ScrolledText(frame_readable, font=("Malgun Gothic", 10), state="disabled", bg="#F9F9F9")
        text_readable.pack(fill="both", expand=True)

        with open(json_path, 'r', encoding='utf-8') as f:
            plan_data = json.load(f)

        readable_content = self.json_to_readable_text(plan_data)
        text_readable.config(state="normal")
        text_readable.insert(tk.END, readable_content)
        text_readable.config(state="disabled")

        # 2. 편집 탭 (읽기 쉬운 형식으로 편집)
        frame_edit = ttk.Frame(notebook)
        notebook.add(frame_edit, text="✏️ 편집")

        text_area = scrolledtext.ScrolledText(frame_edit, font=("Malgun Gothic", 10))
        text_area.pack(fill="both", expand=True)
        text_area.insert(tk.END, readable_content)

        # 3. JSON 탭 (원본 JSON)
        frame_json = ttk.Frame(notebook)
        notebook.add(frame_json, text="⚙️ JSON 원본")

        text_json = scrolledtext.ScrolledText(frame_json, font=("Consolas", 9), state="disabled")
        text_json.pack(fill="both", expand=True)

        json_content = json.dumps(plan_data, indent=4, ensure_ascii=False)
        text_json.config(state="normal")
        text_json.insert(tk.END, json_content)
        text_json.config(state="disabled")

        def save_edits():
            try:
                edited_text = text_area.get("1.0", tk.END).strip()

                # 텍스트 형식에서 JSON으로 변환 시도
                try:
                    # 직접 JSON 파싱 시도
                    updated_data = json.loads(edited_text)
                except json.JSONDecodeError:
                    # 텍스트 형식으로 되어있으면, 원본 구조를 유지하면서 텍스트만 업데이트
                    self.log("ℹ️ 텍스트 형식으로 수정되었습니다. 원본 구조를 유지합니다.")
                    updated_data = plan_data.copy()

                    # 간단한 텍스트 파싱 (제목, 본문 등만 업데이트)
                    lines = edited_text.split('\n')

                    for i, line in enumerate(lines):
                        if '메인 제목:' in line:
                            updated_data['plan']['cover']['main_title'] = line.split('메인 제목:')[1].strip()
                        elif '부제목:' in line:
                            updated_data['plan']['cover']['sub_title'] = line.split('부제목:')[1].strip()
                        elif '페이지' in line and ':' in line:
                            # 본문 페이지 파싱
                            page_num = int(line.split('페이지')[1].split(':')[0].strip()) - 1
                            if page_num < len(updated_data['plan'].get('body', [])):
                                # 다음 라인들에서 내용 추출
                                body_content = []
                                for j in range(i + 1, len(lines)):
                                    if lines[j].startswith('    •'):
                                        body_content.append(lines[j].replace('    •', '').strip())
                                    elif lines[j].startswith('  페이지'):
                                        break
                                if body_content:
                                    updated_data['plan']['body'][page_num]['summary'] = body_content
    ...
    def start_designing(self):
        ...
        json_path = self.plan_json_path.get()
        if not json_path or not os.path.exists(json_path):
            messagebox.showerror("오류", "기획안 파일을 찾을 수 없습니다.")
            return
    ...
    def process_design(self):
        """디자인 로직 실행 (별도 스레드)"""
        try:
            # 새로운 디자인 프로세스 시작시 생성된 이미지 목록 초기화
            ...
            # 새로운 파일명 형식: YYMMDD_MainTitle_PageType.png
            date_str = datetime.datetime.now().strftime("%y%m%d")
            main_title = plan_data["plan"]["cover"]["main_title"]
            # 파일명에 사용 가능한 문자만 유지 (특수문자 제거)
            main_title_clean = re.sub(r'[<>:"/\\|?*]', '', main_title).strip()
            main_title_clean = main_title_clean.replace(' ', '_')

            # 페이지 타입 매핑: 00_cover -> cover, 01_body -> body1, etc.
            def get_page_type_name(suffix):
                if suffix == "00_cover":
                    return "cover"
                elif suffix == "99_outro":
                    return "outro"
                elif "body" in suffix:
                    # 01_body -> body1, 02_body -> body2, etc.
                    num = int(suffix.split('_')[0])
                    return f"body{num}"
                return suffix

            def get_filename(suffix):
                page_type_name = get_page_type_name(suffix)
                return f"{date_str}_{main_title_clean}_{page_type_name}.png"
            ...
            for i, page in enumerate(pages):
                out_path = OUTPUT_DIR_IMAGE / get_filename(page['suffix'])
                if out_path.exists():
                    start_index = i + 1
            ...
            # 첫 Cover 페이지에서 색상 팔레트 추출 (모든 페이지에서 일관되게 사용)
            ...
            for i in range(start_index, len(pages)):
                ...
                if page['type'] == 'COVER':
                    ...
                    if refs and len(refs) > 0:
                        cover_color_palette = refs[0].get('color_palette_feel', tone)
                        self.log(f"  (🎨 색상 팔레트 저장됨: {cover_color_palette})")

                elif page['type'] in ['BODY', 'OUTRO']:
                    # Body/Outro: 같은 색상 팔레트를 가진 다른 레이아웃 검색
                    query = f"{tone} 느낌의 {page['type']} 디자인"
                    self.log(f"  (색상 팔레트를 유지하면서 '{page['type']}' 스타일을 검색하고 있습니다...)")

                    if cover_color_palette:
                        self.log(f"  (🎨 색상 팔레트 필터 적용: {cover_color_palette})")
                        refs = search_rag_references(
                            self.index_data, query, page_type=page['type'],
                            top_k=2, color_filter=cover_color_palette
                        )
                    else:
                        # Fallback: 색상 필터 없이 검색
                        refs = search_rag_references(self.index_data, query, page_type=page['type'], top_k=2)
                ...
                # Cover 이후 페이지들은 색상 팔레트를 명시적으로 지정
                if page['type'] != 'COVER' and cover_color_palette:
                    prompt.append(f"**[색상 팔레트 통일 지시]** 반드시 다음 색상 팔레트를 유지하세요: '{cover_color_palette}'")

                prompt.extend([
                    "**[필수 지시사항]**",
                    "1. 텍스트는 반드시 **한글이 깨지지 않게 디자인과 완벽하게 어우러지도록** 크고 명확하게 배치해야 합니다.",
                    "2. 모든 페이지는 일관된 톤앤매너와 색상 톤을 유지해야 합니다.",
                    ...
                    try:
                        ...
                        if generated_image:
                            ...
                            while out_path.exists():
                                filename = get_filename(page['suffix'])
                                filename_parts = filename.rsplit('.', 1)
                                out_path = OUTPUT_DIR_IMAGE / f"{filename_parts[0]}_{counter}.png"
                                counter += 1
                ...
                if i == start_index and success and not self.stop_design_flag:
                    ...
                    # 생성된 이미지를 세션 목록에 추가
                    if str(out_path) not in self.generated_images:
                        self.generated_images.append(str(out_path))
