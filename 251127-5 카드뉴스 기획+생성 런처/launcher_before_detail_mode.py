  ...
  "estimated_tone": "(예: 활기찬, 정보성)"
    ...
    def process_design(self):
        ...
        try:
            ...
            # Body 페이지용 레퍼런스 (처음 1번만 검색, 이후 재사용)
            body_refs = None
            ...
            for i in range(start_index, len(pages)):
                ...
                # 참조 이미지 검색
                if page['type'] == 'BODY' and body_refs is not None:
                    # Body 페이지: 이미 검색한 레퍼런스 재사용
                    self.log(f"  (저장된 Body 스타일을 사용합니다.)")
                    refs = body_refs
                else:
                    # Cover, Outro: 매번 새로 검색 / Body: 처음 1번만 검색
                    ...
                    # Body 페이지 첫 검색 결과 저장
                    if page['type'] == 'BODY' and body_refs is None:
                        body_refs = refs
